#' Fitting Linear Models with Better Subset Algorithm
#'
#' @description Implementation of the MM algorithm solver for the better subset
#' algorithm for a linear regression model.
#'
...
#' function is called.
#' @param m number of explanatory variables to include in the model.
...
better.subset.lm = function(formula, data, m, intercept=T, standardize=F,
  beta.init=NULL, beta.tol=0, loss.tol=1e-6, seed=NULL, verbose=0) {

  # Extract dependent variable and explanatory variables
  y = data.matrix(data[, all.vars(formula)[1]]);
  x = stats::model.matrix(formula, data); rss.tot = sum((y - mean(y)) ^ 2)
  ...
  # Define constants
  Xt.X = crossprod(x); inv.lambda = 1 / eigen(Xt.X)$values[1]; set.seed(seed)
  N = nrow(x); P = ncol(x); inv.lambda.Xt.y = inv.lambda * crossprod(x, y)

  # Define helper function for displaying progress
  ...
  i = 0L; while (TRUE) { i = i + 1L; l.old = l.new
    ...
    u = b.new - inv.lambda * (Xt.X %*% b.new) + inv.lambda.Xt.y
    abs.u = abs(u); abs.u.max = sort(abs.u, decreasing=T)[m]
    b.new = ifelse(abs.u >= abs.u.max, u, 0)
